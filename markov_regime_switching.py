...
data["state"] = data["returns"].apply(
    lambda x: "up" if (x > 0.001) else ("down" if (x < -0.001) else "no_change")
)
print(data.tail())
...
model = sm.tsa.MarkovRegression(
    data["returns"][-2500:], k_regimes=3, switching_variance=True
)
...
fig.add_trace(
    go.Scatter(x=com_1["Date"], y=com_1["Predicted"], name="Predicted return")
)

fig.update_layout(
    title="Nasdaq Actual vs Predicted returns",
    yaxis_title="Price ($)",
...
fig.add_trace(
    go.Scatter(
        x=combine["Date"],
        y=combine["forecast_variable"],
        name="Actual Values (Adj Close",
    )
)

fig.add_trace(
    go.Scatter(x=combine["Date"], y=combine["Predicted"], name="Predicted return")
)

fig.update_layout(
    title="Nasdaq Actual vs Predicted values",
    yaxis_title="Price ($)",
    font=dict(family="Courier New, monospace", size=18, color="#7f7f7f"),
)
fig.update_layout(
